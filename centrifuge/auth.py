...
def check_sign(secret, project_name, encoded_data, auth_sign):
    ...
    if not hash_algorithm:
        return False
    sign = hmac.new(six.b(str(secret)), digestmod=hash_algorithm)
    sign.update(six.b(project_name))
    sign.update(six.b(encoded_data))
    return sign.hexdigest() == auth_sign


def get_client_token(secret, project_name, user, timestamp, user_info=None, hash_algorithm=None):
    """
...
def check_client_token(token, secret, project_name, user, timestamp, user_info=None):
    ...
    if not hash_algorithm:
        return False
    client_token = get_client_token(secret, project_name, user, timestamp, user_info, hash_algorithm=hash_algorithm)
